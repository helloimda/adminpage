...
import { useMemberReportCount, useMemberReportList } from '@/hooks/use-report';

export function MemberReportTable(): React.JSX.Element {
  const [currentPage, setCurrentPage] = useState(0);
  const [viewMode, setViewMode] = useState<'normal' | 'byReportCount'>('normal');
  const rowsPerPage = 10;

  const { data: reportData, loading: listLoading, error: listError } = useMemberReportList(currentPage + 1);
  const { data: countData, loading: countLoading, error: countError } = useMemberReportCount();
  ...
  const mergedData = useMemo(() => {
    ...
    return reportData.data.map((report) => {
      const countItem = countData.find((item) => item.gd_idx === report.gd_idx);
      return {
        ...report,
          ...
          <TableBody>
            {listLoading || countLoading ? (
              ...
              mergedData.map((report) => (
                <TableRow key={report.gd_idx}>
                  <TableCell>{report.gd_idx}</TableCell>
