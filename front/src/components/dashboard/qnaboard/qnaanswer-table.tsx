...
import { useAnsweredQnaList } from '@/hooks/board/use-qna'; // useAnsweredQnaList 훅 가져오기
import { useSelection } from '@/hooks/use-selection';

export function AnsweredQnaTable(): React.JSX.Element {
  const [currentPage, setCurrentPage] = React.useState(0); // 페이지네이션의 기본 페이지를 0으로 설정
  const rowsPerPage = 10;

  const router = useRouter();

  const { data, loading } = useAnsweredQnaList(currentPage + 1); // 페이지는 1부터 시작하기 때문에 +1
              ...
              data.data.map((row) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell>{dayjs(row.rdt).format('YYYY-MM-DD HH:mm')}</TableCell>
